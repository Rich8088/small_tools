...
# --------------------1、淘客结算信息处理及存储到数据库--------#
# --读取原始数据并处理--#
df_taoke = read_csv_file_to_dataframe(rootDir, '结算明细', missing_values, 0, coding='utf-8')
...
# ----字段处理
df_taoke.drop(['商品名称', '服务费率', 'index', '文件名', '路径', '佣金比例'], axis=1, inplace=True)
df_taoke = df_taoke[
    ['店铺名', '来源或淘客昵称', '团长名称', '计划名称', '创建时间', '确认收货时间', '淘客结算时间', '商品ID',
     '成交商品数', '实际成交价格', '佣金', '服务费金额', '淘宝父订单编号', '淘宝子订单编号']]
...
# --将原始数据存入数据库
type_dict_order = mapping_df_types(df_taoke)
df_taoke.to_sql(name='taoke_info', con=engine, if_exists='append', index=False,
                dtype=type_dict_order)
# print(df_taoke.info())

# --------------------2、淘客维权信息处理及存储到数据库--------#
# --读取原始数据并处理--#
df_taoke_refund = read_csv_file_to_dataframe(rootDir, '维权', missing_values, 0, coding='utf-8')
df_taoke_refund = df_taoke_refund.rename(
    columns=lambda x: x.replace("'", "").replace('"', '').replace(" ", ""))
df_taoke_refund.dropna(axis=0, subset=['维权完成时间'], inplace=True)
# --提取文件路径和下载时间信息
...
# --字段处理
df_taoke_refund.drop(['商品名称', 'index', '文件名', '路径', '维权创建时间'], axis=1,
                     inplace=True)
df_taoke_refund = df_taoke_refund[['店铺名', '淘宝订单编号', '淘宝子订单编号', '维权状态', '订单结算时间',
                                   '维权完成时间', '维权退款金额', '应退回服务费', '应退回佣金']]
df_taoke_refund.sort_values(by=['淘宝子订单编号', '维权完成时间'], ascending=True, inplace=True)
# 格式转化
...
# 保留最新的数据记录
df_taoke_refund.sort_values(by=['淘宝订单编号', '淘宝子订单编号', '维权完成时间'], ascending=True, inplace=True)
df_last_taoke_refund = df_taoke_refund.drop_duplicates(subset=['淘宝子订单编号'], keep='last')
df_last_taoke_refund = df_last_taoke_refund.reset_index(drop=True)
# --将原始数据存入数据库
type_dict_taoke_refund = mapping_df_types(df_last_taoke_refund)
df_last_taoke_refund.to_sql(name='taoke_refund', con=engine, if_exists='append', index=False,
